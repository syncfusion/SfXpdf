...
int mainHtmlExport(int argc, char *argv, int firstPage, int lastPage, char *outputFileName,bool complexmode,bool noFrame,bool ignoreImages, bool outputHiddenText,float zoom,char *ownerPwd,char *userPwd,void *stream, void *pdfDoc) {
  ...
  ignore=ignoreImages;
  scale=zoom;
//  showHidden=outputHiddenText;

  strncpy((char *)ownerPassword, ownerPwd, 127);
   ((char *)ownerPassword)[127] = '\0';
   strncpy((char *)userPassword, userPwd, 127);
   ((char *)userPassword)[127] = '\0';
//  // parse args
...
//  // construct text file name
 /* if (argc == 3) {*/
    GString* tmp = new GString(outputFileName);
