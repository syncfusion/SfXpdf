...
// get CSS font name for font #i 
GString* HtmlFontAccu::getCSStyle(int i, GString* content,int j){
  GString *tmp;
  GString *iStr=GString::fromInt(i);
  
  if (!xml) {
    tmp = new GString("<div class=\"ft");
    tmp->append(iStr);
	tmp->append(" p");
	iStr=GString::fromInt(j);
	tmp->append(iStr);
    tmp->append("\">");
    tmp->append(content);   
	tmp->append("</div>\n");
...
GString* HtmlFontAccu::getCSStyle(int i, GString* content,int j,bool isMultiColumn,int marginTop,int pageWidth){
  GString *tmp;
  GString *iStr=GString::fromInt(i); 
  if (!xml) {
    tmp = new GString("<div style=\"width:");
	iStr=GString::fromInt(pageWidth);
    tmp->append(iStr);
	tmp->append("px");
	tmp->append(";float:left;border:none;margin:");
	iStr=GString::fromInt(marginTop);

	tmp->append(iStr);
	tmp->append("px 0px 0px 0px;padding: 0px;border:none;overflow: hidden;\">\n");
	tmp->append("<p style=\"margin-top: 0px\" class=\"ft");
	iStr=GString::fromInt(i);
	tmp->append(iStr);
	tmp->append(" p");
	iStr=GString::fromInt(j);
	tmp->append(iStr);
...
GString* HtmlFontAccu::getMultiColumnCSStyle(int i, GString* content,int j,int coumnIndex,int marginTop,int lineHeight){
  GString *tmp;
  GString *iStr=GString::fromInt(i);
  ...
  if (!xml)
  {
	 ...
	 if(coumnIndex>0)
	  {
		tmp->append("style=\"padding-left:0px;margin-top: ");
		iStr=GString::fromInt(marginTop);
		tmp->append(iStr);
		tmp->append("px;");
		tmp->append("height: ");
		iStr=GString::fromInt(lineHeight);
		tmp->append(iStr);
		tmp->append("px;\"");
	  }
	  tmp->append(" class=\"ft");
...
GString* HtmlFontAccu::getColumnCSStyle(int i, GString* content,int j,int paddingLeft,int columnIndex,int marginTop,int lineheight){
  GString *tmp;
  GString *iStr=GString::fromInt(i);
  ...
  if (!xml)
  {
	  ...
	  if(columnIndex>0)
	  {
		tmp->append("style=\"padding-left: ");
		iStr=GString::fromInt(paddingLeft);
		tmp->append(iStr);
		tmp->append("px;");
		tmp->append("Height: ");
		iStr=GString::fromInt(lineheight);
...
GString* HtmlFontAccu::getMultiColumnCSStyle(int i, GString* content,int j,int coumnIndex,int marginTop,bool b,int lineHeight,int paddingLeft){
  GString *tmp;
  GString *iStr=GString::fromInt(i);
...
//GString* HtmlFontAccu::getMultiColumnCSStyle(int i, GString* content,int j){
//	 GString *tmp;
//  GString *iStr=GString::fromInt(i);
//  
//  if (!xml)
...
//	  tmp = new GString("<p ");
//	
//		 tmp->append("style=\"padding-left:0px\"");
//		 /*tmp->append("style=\"margin-top: ");*/	
//		  
//	
//	  tmp->append(" class=\"ft");
...
//
//}
GString* HtmlFontAccu::getIntersectColumnCSStyle(int i, GString* content,int j,int coumnIndex,int marginTop,int paddingLeft){
...
GString* HtmlFontAccu::CSStyle(int i){
   ...
   GString *fontName=font.getFullName();//font.getFontName();
   
   GString *s=new GString("");
   char *f=new char[fontName->getLength()];
   int temp=0;
   bool isTrue=false;
   for(int i=0;i<fontName->getLength();i++)
   {
	   f[i]=fontName->getChar(i);
	   if(isTrue)
	   {
		   s->append(f[i]);
	   }
   ...
   if(!xml){
     tmp->append(".ft");
     tmp->append(iStr);
     tmp->append("{vertical-align:top;font-size:");
     tmp->append(Size);
