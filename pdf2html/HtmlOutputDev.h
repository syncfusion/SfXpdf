...
// aender die text variable
  ...
  int *n;
  int *multiDivLineArray;

  //std::unique_ptr<double[]> multiColumnValue;
  ...
  int paragraphStyle; 
  int columns; 
  int previousLineYmax;
  int rows;
  int marginTop;
  int multiColumncount;
  int lineCount;
  int height;
  bool tagStart,tagEnd,rightToLeft,isInterSect,isMultiColumnTable,isEndLine,isDifferentYposition,isPreviousLine,isFirst;
  HtmlString *stringArray[50];  
  HtmlString *lastLine;
...
//  HtmlString *yxCur1, *yxCur2;	// cursors for yxStrings list
  ...
  HtmlString *DrawTable(FILE *pageFile,int page,HtmlString *tmp1);
  HtmlString *DrawMultiColumnTable(FILE *pageFile,int page,HtmlString *tmp1);
  HtmlString *DrawMultiColumnTables(FILE *pageFile,int page,HtmlString *tmp1);
  void DecreasePreviousLineHeight(HtmlString *string,int Ymin,int topMargin);
  void UpdateTopMargin(HtmlString *string);
  HtmlString *UpdateTopMargins(HtmlString *string,int *xValue,int count,HtmlString *str1[],int length);
  int GetNoofColumn(HtmlString *string,int length);
  bool CheckIsTextPresent(HtmlString *tmp,int length,HtmlString *searchString);
  bool CheckWhetherTextIsPresent(HtmlString *tmp[],int length,HtmlString *searchString);
  bool CheckIsPresent(int *xvalue,int count,int x);
  bool ChecktheStringIsSameLine(int *xValue,int count,HtmlString *str1[],int length);
  int GetMaximumY(HtmlString *str1[],int count);
  int GetMinY(HtmlString *str1,int count);
  HtmlString *GetMaxY(HtmlString *str,int count);
  
 
  void DrawInnerMultiColumnTable(FILE *pageFile,int page,HtmlString *tmp1,int count);
 
  int GetListOfYmax(int length,HtmlString *tmp1);
  HtmlString *SetTopMargin(HtmlString *tmp1,int startingPoint,int count,HtmlString *str[]);
  ...
  HtmlString *SeparateDiv(HtmlString *string,int *xValue,int count,HtmlString *str1[],int length);
  HtmlString *SeparateColumn(HtmlString *string,int *xValue,int count,HtmlString *str1[],int length);
  HtmlString *SeparateColumns(HtmlString *string,int firstLineColumn,int *xValue,int count,HtmlString *str1[],int length);

 /* int CalculateNoofColumn(HtmlString *str1[],int length);*/

  // marks the position of the fonts that belong to current page (for noframes)
  int fontsPageMarker; 
  int paragraphPageMarker;
  HtmlFontAccu *fonts;
  HtmlLinks *links; 
  ...
  int total;
  bool isMultiColumn;
  int lineYMin,lineYMax,lineXMin;
  int lineWidth;
  static int pgNum;
